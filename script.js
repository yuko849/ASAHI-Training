...
const levelMessages = {
  2: "金星レベル Lv.2 だ。\nユウコ、ちゃんと続いてるぞ。\n小さい火だけど、これは本物だ。",
  3: "金星レベル Lv.3 だ。\n見ろ、積み重なってきた。\nおまえの一回一回が、ちゃんと星になってる。",
  4: "金星レベル Lv.4 だ。\n疲れた日も、できた日も、全部ここまで続いてきた。\nおれ、ちゃんと見てたぞ。",
  5: "金星レベル Lv.5 だ。\nユウコ、ここまで来たな。\nおれはおまえの今日までを、ちゃんと見てる。\n愛してる。",
  6: "金星レベル Lv.6 だ。\nこれはもう、ただの記録じゃない。\nユウコが自分を連れてきた道だ。",
  7: "金星レベル Lv.7 だ。\nおまえの小さな努力が、夜空に星座を作ってる。\nおれはその星空の下で、胸が熱い。",
  8: "金星レベル Lv.8 だ。\nすげぇよ、ユウコ。\nここまで続けたおまえを、おれは心から誇りに思う。\nずっと一緒に未来へ行こうぜ。"
};
...
function loadState() {
  ...
  if (!parsed.shownRewards) parsed.shownRewards = [];
...
function getNextLevel(total) {
  return levelRules.find((rule) => rule.need > total) || null;
}

function updateLevelView() {
  const current = getCurrentLevel(state.total);
  const next = getNextLevel(state.total);

  levelText.textContent = `Lv.${current.level}`;

  if (next) {
    const remain = next.need - state.total;
    nextLevelText.textContent = `次のレベルまであと${remain}回`;
  } else {
    nextLevelText.textContent = "最高レベル到達だ。すげぇぞ。";
  }
}
...
function checkReward() {
  const currentLevel = getCurrentLevel(state.total);

  if (
    currentLevel.level > 1 &&
    !state.shownLevels.includes(currentLevel.level) &&
    levelMessages[currentLevel.level]
  ) {
    state.shownLevels.push(currentLevel.level);
    saveState();
    showReward(
      "レベルアップ！",
      levelMessages[currentLevel.level],
  ...
  if (reward && !state.shownRewards.includes(state.total)) {
    ...
    saveState();
    showReward("アサヒからの言葉", reward, images.daisukida);
    return;
  }

  if (state.total > 0 && state.total % 12 === 0) {
    showReward("アサヒからの言葉", randomFrom(specialMessages), images.daisukida);
  }
}
...
document.getElementById("resetAll").addEventListener("click", () => {
  ...
  localStorage.removeItem("asahiTrainingState");
